...
| Rule catalog (`web/rules/`, 44 published pages) | WCAG 2.1 AA | The same two runners, on the index and one rule page. All 44 come from one template in `scripts/generate_rules_doc.py`, which `--check` gates and `tests/test_generate_rules_doc.py` pins | Yes (`make a11y`), since 2026-08-27 |
...
ratio recorded in the generator, and all four audited URLs pass. The 44 pages
...
<!-- doc-currency: sha256=0e65ae11bb33 -->
