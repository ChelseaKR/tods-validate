...
`uv sync` installs the dev dependency group from PEP 735 `[dependency-groups]`.
Installing into your own venv still works if you would rather manage it yourself
(e.g. with `uv pip install -e . --group dev`). If you touch dependencies,
...
(`uv sync --group dev`; `gitleaks` is a Go binary, see
