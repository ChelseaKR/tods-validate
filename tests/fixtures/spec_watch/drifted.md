...
A trimmed, deliberately drifted fork of `in_sync.md`, used by
`tests/test_spec_watch.py`. Exactly one field changed from the
`tods_validate.schema.TABLES` transcription: `vehicles.txt`'s
`vehicle_label` is `Required` here instead of `Optional`, so running
`spec_watch.py` against this file reports that single presence drift.
...
| `vehicle_label` | Text | Required | Free text label for a vehicle. |
