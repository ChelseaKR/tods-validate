...
# registry, which is populated by import time.
...
def render_text(  # noqa: C901 -- pragmatic complexity; ratchet tracked in docs/CONFORMANCE-GAPS.md#code-quality
...
def render_markdown(  # noqa: C901 -- pragmatic complexity; ratchet tracked in docs/CONFORMANCE-GAPS.md#code-quality
    ...
    *,
    ...
    if stamp:
        ...
        if coverage is not None:
            lines.append(
                f"_Rule-set coverage: {len(coverage.ran)} of {len(coverage.outcomes)} rules "
                f"ran, {len(coverage.skipped)} skipped._"
            )
    return "\n".join(lines)
...
def _sarif_descriptor(rule_id: str, level: str) -> dict[str, object]:
    ...
    The registry already carries a title, a description, and a spec link for
    every rule; earlier versions dropped all of it and emitted a bare id. Here
    they become ``shortDescription``, ``fullDescription``, and ``helpUri`` so a
    dashboard can show what a rule means and link the spec.
    """
    ...
    if rule is not None:
        ...
        descriptor["helpUri"] = rule.spec_section
        descriptor["properties"] = {"category": rule.category, "severity": rule.severity.name}
    return descriptor
...
def render_sarif(
    ...
    tool's ``rules`` array, enriched from the rule registry (title, description,
    spec link); each finding becomes a ``result`` pointing at the file and
    1-based line, with its structured ``data`` carried in ``result.properties``.
